...
func (bd *BlockDevice) WritePartitionTable(legacyBios bool, wholeDisk bool, dryRun *[]string) error {
	...
	// Not all children can be partitioned using Parted.
	// For e.g: If the bd.Fstype to "LVM2_member", their children have
	// type as BlockDeviceTypeLVM2Volume, they will be virtual partitions(LV)
	// whose creation mechanics differs from that of regular partition.
	// Once partitioned, they can formatted just like another other file system using MakeFs
